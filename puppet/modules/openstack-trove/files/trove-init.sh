...
keystone endpoint-create --region=ifsc-sj --service-id=$(keystone service-list | awk '/ trove / {print $2}') --publicurl=http://trove:8779/v1.0/%\(tenant_id\)s --internalurl=http://trove:8779/v1.0/%\(tenant_id\)s --adminurl=http://trove:8779/v1.0/%\(tenant_id\)s
